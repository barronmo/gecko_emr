...
class Problems(wx.Panel):
	...
	if dlg.ShowModal() == wx.ID_OK:
	    ...
	    newICD = EMR_utilities.getData('SELECT icd10 FROM icd10 WHERE short_des = "%s";' % self.problem)
...
def listBilledICD(pt_ID):
    ...
    problems = []
    for i in range(1,11):
	qry = "SELECT icd%d FROM notes INNER JOIN demographics USING (patient_ID) WHERE date LIKE '2015%%' AND patient_ID = %s AND icd%d != '';" % (i,pt_ID,i)
...
class AddProblemDialog(wx.Dialog):
	...
	qry = 'SELECT * FROM icd10 WHERE short_des LIKE "%%%s%%";' % (event.GetString())
	...
	qry1 = '(SELECT * FROM icd10 WHERE short_des LIKE "%%%s%%")' % self.textctrl['Search'].GetValue()
